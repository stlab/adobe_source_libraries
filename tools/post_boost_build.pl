...
die "usage: post_boost_build build-number username ldap-password" unless @ARGV == 3;
...
for my $platform ("win32", "macosx") {
    my $Results = $Codex->AddBuild(manifestfile => "VersionInfo.xml",
                                   uri => "p4://boxer.corp.adobe.com:1770//boost_libraries", 
                                   certlevel => "Release 1_36_0 with ASL patches from $build_number build", 
