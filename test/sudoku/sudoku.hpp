...
template <typename Iterator>
class row_iterator_adaptor
    : public boost::iterator_adaptor<row_iterator_adaptor<Iterator>, Iterator> {
public:
    typedef typename boost::iterator_adaptor<row_iterator_adaptor<Iterator>, Iterator>::reference
    reference;
...
template <typename Iterator>
class col_iterator_adaptor
    : public boost::iterator_adaptor<col_iterator_adaptor<Iterator>, Iterator> {
public:
    typedef typename boost::iterator_adaptor<col_iterator_adaptor<Iterator>, Iterator>::reference
    reference;
...
template <typename Iterator>
class box_iterator_adaptor
    : public boost::iterator_adaptor<box_iterator_adaptor<Iterator>, Iterator> {
public:
    typedef typename boost::iterator_adaptor<box_iterator_adaptor<Iterator>, Iterator>::reference
    reference;
...
inline std::ostream& operator<<(std::ostream& s, const possibility_set_t& x) {
    ...
    for (possibility_set_t::const_iterator first(x.begin()), last(x.end()); first != last;
         ++first) {
        ++count;
        if (*first)
            s << " " << count;
...
public:
    ...
    box_iterator box_begin(int box) {
        std::pair<int, int> c(box_begin_coord(box));
        return at(c.first, c.second);
    }
    box_iterator box_end(int box) {
        std::pair<int, int> c(box_end_coord(box));
        return at(c.first, c.second);
    }
...
private:
    static std::pair<int, int> box_begin_coord(int box) {
        ...
        default:
            throw std::runtime_error("box_begin_coord");
        }
    }
    ...
    static std::pair<int, int> box_end_coord(int box) {
        ...
        case 8:
            return std::make_pair(9, 6);
        default:
            throw std::runtime_error("box_end_coord");
        }
    }
