...
\c arg_stream is a namespace wrapping together the concept of a stream from which you can retrieve items (ie function arguments) of various types.
...
arg_stream implementors can communicate that they have an eof function by specializing \c arg_stream::traits<T>::has_eof_memberfunction.
\code
...
\brief defines any traits that help with the implementation of arg_stream::call() and/or helper objects like arg_stream::chain.

\par Currently only communicates if the type T has a member function eof() (which returns whether there are any arguements left in the stream).
...
\par
	...
	If not specialized, the default implementation attempts to call the arg_stream's member function eof() if one is available.
	If there is no member function, it returns false - ie if it is not known, then it is assumed that more args exist, and
...
\par
	...
	ie If not specialized, this default implementation attempts to call the arg_stream's member function get_next_arg<T>().
...
\par This is the point of arg_stream.  To call a function with typed-args pulled out of some abstracted object.
*/
template <typename F, typename ArgStream>
typename result_type<F>::type
...
\brief chain 2 arg_streams together by calling the first stream until depleted, then calling the second.
...
\par this is useful (in combination with arg_stream::chain) when you need to slip a single value at the front/back of an arg_stream.
*/
template <typename T>
struct single
{
