...
\par
Unfortunately C++ doesn't have full inspection (yet) so we need to use 2 macros to implement
ADOBE_HAS_TYPE.
...
Internally it defines a template class, thus it must be used only at global, namespace, or class
scope.
See ADOBE_HAS_TYPE.
*/
#define ADOBE_HAS_TYPE_IMPL(TypeInQuestion)                                                        \
    ...
    struct has_type##TypeInQuestion {                                                              \
        ...
        static const bool value =                                                                  \
            sizeof(SFINAE<C##TypeInQuestion>(0)) == sizeof(adobe::detail::yes_struct);             \
    }

/*!
\ingroup inspection

\brief returns true iff \c C has an internal type named 'TypeInQuestion'.  ie returns true iff
C::TypeInQuestion is a type.

\par
Unfortunately C++ doesn't have full inspection (yet) so we need to use macros to implement
ADOBE_HAS_TYPE.
In order to use this macro, you also need to declare its implementation (for each TypeInQuestion)
using ADOBE_HAS_TYPE_IMPL.
eg: \code
...
*/
#define ADOBE_HAS_TYPE(C, TypeInQuestion) has_type##TypeInQuestion<C>::value
...
#define ADOBE_HAS_MEMBER_IMPL(Member)                                                              \
    ...
    struct has_member##Member {                                                                    \
        __if_exists(Class::Member) { static const bool value = true; }                             \
        __if_not_exists(Class::Member) { static const bool value = false; }                        \
    }
#else
...
Unfortunately C++ doesn't have full inspection (yet) so we need to use macros to implement
ADOBE_HAS_MEMBER.
In order to use this macro, you also need to declare its implementation (for each MemberInQuestion)
using ADOBE_HAS_MEMBER_IMPL.
eg: \code
...
Also, make sure the 2 macros are in the same namespace!
*/
#define ADOBE_HAS_MEMBER(C, MemberInQuestion) has_member##MemberInQuestion<C>::value


/*!
\ingroup inspection

\brief Implementation part of \c ADOBE_HAS_TEMPLATE1 macro.   Required before using
ADOBE_HAS_TEMPLATE1.
...
#define ADOBE_HAS_TEMPLATE1_IMPL(TemplateInQuestion)                                               \
    template <typename C##TemplateInQuestion>                                                      \
    struct has_template1##TemplateInQuestion {                                                     \
        ...
        static adobe::detail::no_struct SFINAE(...);                                               \
        static const bool value =                                                                  \
            sizeof(SFINAE<C##TemplateInQuestion>(0)) == sizeof(adobe::detail::yes_struct);         \
    }

/*!
\ingroup inspection

\brief returns true iff \c C has an internal template named 'TemplateInQuestion' with 1 (nondefault)
template param.  ie returns true iff "C::template TemplateInQuestion<someType>" is a syntactically
correct.
...
TemplateInQuestion) using ADOBE_HAS_TEMPLATE1_IMPL.
eg: \code
...
we could make even more macros for the constant value case(s), and all the permutations... if need
be
Also, make sure the 2 macros are in the same namespace!
*/
#define ADOBE_HAS_TEMPLATE1(C, TemplateInQuestion) has_template1##TemplateInQuestion<C>::value
...
#define ADOBE_HAS_TEMPLATE2(C, TemplateInQuestion) has_template2##TemplateInQuestion<C>::value
