...
template <typename R> // R models FullorderRange
inline boost::iterator_range<depth_fullorder_iterator<typename boost::range_iterator<R>::type>>
depth_range(R& x) {
...
\param destIter iterator to the location in destForest where the transformed elements should be
inserted
...
void transform_forest(SourceForestIterType first, SourceForestIterType last,
    ...
    while (first != last) {
        if (first.edge() == forest_leading_edge) {
            auto insertedIter =
                destForest.insert(!insertionIterStack.empty() ? insertionIterStack.top() : destIter,
                                  transformFunc(*first));
...
template <typename SourceForestIterType, typename DestForestType, typename UnaryFunction>
void transform_forest(SourceForestIterType first, SourceForestIterType last,
                      DestForestType& destForest, UnaryFunction transformFunc) {
...
\param sourceForest forest containing the elements to be transformed
\param destForest forest that the transformed elements should be inserted into
\param destIter iterator to the location in destForest where the transformed elements should be
inserted
\param transformFunc function for transforming elements in the range
...
void transform_forest(SourceForestType sourceForest, DestForestType& destForest,
                      typename DestForestType::iterator destIter, UnaryFunction transformFunc) {
...
void transform_forest(SourceForestType sourceForest, DestForestType& destForest,
                      UnaryFunction transformFunc) {
    transform_forest(sourceForest.begin(), sourceForest.end(), destForest, destForest.end(),
                     transformFunc);
}
